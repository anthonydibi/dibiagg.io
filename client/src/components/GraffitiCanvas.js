...
import { Flex, IconButton, Box, Heading, Grid, GridItem, useBreakpointValue, Stack, Skeleton, Input, InputGroup, Text, InputRightElement, useColorModeValue, useDisclosure } from '@chakra-ui/react'
...
export default function GraffitiCanvas() { //built off of free-draw template from react-konva docs
    ...
    const stageScale = useBreakpointValue({ base: window.innerWidth/1000.0, md: 1 })
    ...
    const handleMouseDown = (e) => {
        if(step === 0){
            ...
            const newLine = { tool, color: color.hex ?? color, points: [pos.x/stageScale, pos.y/stageScale], who: userTag };
    ...
    const handleMouseMove = (e) => {
        ...
        if(intersection && intersection.attrs.globalCompositeOperation !== "destination-out"){
            if(intersection.attrs.hasOwnProperty("who") && intersection.attrs.who !== ""){
  ...
  return (
    <Box className='GraffitiContainer' w="100%">
        ...
        <Stack direction={{base: "column", md: "row"}} w={"100%"} align={{base: "center", md: "none"}}>
        <InputGroup size='md' w="95%" display={{base: "block", md: "none"}} mt="2">
            <Input
                pr='4.5rem'
                type={'text'}
            ...
            <InputRightElement width='4.5rem' mr="-3">
                <IconButton size="sm" isRound="true" value="taggingHelp" variant="interact" icon={<AiOutlineQuestion/>} onClick={onOpen}>
                </IconButton>
            </InputRightElement>
        </InputGroup>
        <Flex alignItems={"center"} w={"100%"}>
            <Stack flex={"1"} justify={"right"} align={"center"} direction={"row"}>
                <InputGroup size='md' w="400px" m="auto" display={{base: "none", md: "block"}}>
                    <Input
                        pr='4.5rem'
                        type={'text'}
                        placeholder="Enter tag"
                ...
                <IconButton size="sm" isRound="true" value="previousDay" variant="interact" icon={<AiFillCaretLeft/>} onClick={ back }>
                </IconButton>
            </Stack>
            <Heading my={"2"} mx={"2"}>{day}</Heading>
            <Box flex={"1"}>
            <IconButton size="sm" isRound="true" value="nextDay" variant="interact" icon={<AiFillCaretRight/>} onClick={ next }>
            </IconButton>
            {
                step > 0 ?
        ...
        <Flex direction={{base: "column", md: "row"}}>
            ...
            <Skeleton position={"relative"} isLoaded={isLoaded} zIndex={"5"} border="1px solid" borderBottom={{base: "1px", md: "0px none"}}>
                <Text color={"black"} fontSize={"lg"} position={"absolute"} m={"2"} top={"0"} left={"0"}>
                        { hoveredTag }
                </Text>
                <Stage
